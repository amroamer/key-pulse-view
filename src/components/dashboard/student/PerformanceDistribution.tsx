...
const PerformanceDistribution = () => (
  <div className="rounded-xl border border-border bg-card p-4 space-y-3">
    <div>
      ...
      <p className="text-[10px] text-muted-foreground mt-0.5">Average assessment scores · Target: 80</p>
    </div>
    <div className="h-[220px]">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 10, right: 10, left: -15, bottom: 0 }}>
          <XAxis dataKey="name" tick={{ fontSize: 10, fill: "hsl(168, 10%, 45%)" }} tickLine={false} axisLine={false} />
          <YAxis tick={{ fontSize: 10, fill: "hsl(168, 10%, 45%)" }} tickLine={false} axisLine={false} domain={[0, 100]} />
          <Tooltip contentStyle={{ fontSize: "11px", borderRadius: "8px", border: "1px solid hsl(160, 10%, 90%)" }} />
          <ReferenceLine y={80} stroke="hsl(155, 75%, 42%)" strokeDasharray="4 4" label={{ value: "Target", position: "insideTopRight", fontSize: 9, fill: "hsl(155, 75%, 42%)" }} />
