...
const PillarSummary = () => {
  const pillars = Object.entries(pillarNames).map(([code, name]) => {
    ...
    const overallStatus = avgScore >= 1.5 ? "green" : avgScore >= 0.5 ? "amber" : "red";
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2.5">
      {pillars.map((p) => (
        <div
          key={p.code}
          className={`rounded-xl border p-4 ${statusBg[p.overallStatus as keyof typeof statusBg]} transition-shadow hover:shadow-md cursor-pointer group`}
        >
          <div className="flex items-center gap-2 mb-2">
            <span className={`w-2.5 h-2.5 rounded-full ${statusDot[p.overallStatus as keyof typeof statusDot]}`} />
